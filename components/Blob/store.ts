...
export function parseState(search = localWindow?.location.search) {
...
export const useUIStore = create((set) => ({
  ...
  isGallery: false,
  isRemix: false,
  targetX: localWindow?.innerWidth / 2,
  currentPage: 0,
  ...
  gotoPage: (nextPage) =>
    set((state) => {
      const pageWidth = localWindow?.innerWidth * state.pageWidthRatio;
      const margin = (localWindow?.innerWidth - pageWidth) * 0.5;
